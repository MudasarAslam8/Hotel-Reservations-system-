<?php
	...
	if(ISSET($_POST['add_guest'])){
	    ...
	    $CNIC = $_POST['CNIC'];
		$checkin = $_POST['date'];
		$conn->query("INSERT INTO `guest` (firstname,  lastname, City, contactno, CNIC) VALUES('$firstname',  '$lastname', '$city', '$contactno','$CNIC')") or die(mysqli_error());
		$query = $conn->query("SELECT * FROM `guest` WHERE `firstname` = '$firstname' && `lastname` = '$lastname' && `city` = '$city' && `contactno` = '$contactno' &&`CNIC` = '$CNIC'") or die(mysqli_error());
